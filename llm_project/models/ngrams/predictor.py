...
def predict_next_word(words: list, ngram_freqs, n):
    ...
    if len(words) == n - 1:
        for ngram, frequency in ngram_freqs.items():
        ...
        if ngram_pred:
            ...
            best_ngram = max(ngram_freqs, key=ngram_freqs.get)
            prediction = best_ngram[-1]
            print("No ngram found to match the word, fallback to most frequent ngram's final word")
    else:
        raise ValueError("The word provided does not match the maximum ngram length")
    return prediction
