...
def main(max_iters, embd_dim, n_layer, dropout, max_k, device, force_retrain=False):
    ...
    print(f"[sanity] raw chars in file: {len(text):,}")
    ...
    try:
        ...
        recon_ok = False
    print(f"[sanity] decode(encode(text)) == original? {recon_ok}")

    TOTAL_TOKENS = len(data)
    print(
        f"[sanity] total tokens (after BPE): {TOTAL_TOKENS:,}  | vocab_size: {vocab_size}"
    )
    ...
    # Coverage of starting positions (ADD)
    ...
    model_save_path = os.path.join(save_path, f"gpt_shakespeare{debug_suffix}.pth")
    print(f"Saving model to {model_save_path}")
    torch.save(model.state_dict(), model_save_path)
